...
class DistilBert_batchnorm_pblstm(DistilBertPreTrainedModel):
...
class DistilBert_pblstm(DistilBertPreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = distilbert_output[0]  # (bs, max_query_len, dim)
        lstm_output, _ = self.blstm(hidden_states)
        hidden_states = self.dropout(lstm_output)  # (bs, max_query_len, dim)
        hidden_states = self.pblstm(hidden_states)
        logits = self.qa_outputs(hidden_states)  # (bs, max_query_len, 2)
        start_logits, end_logits = logits.split(1, dim=-1)
        start_logits = start_logits.squeeze(-1)  # (bs, max_query_len)
